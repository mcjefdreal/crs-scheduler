...
<section class="rounded-md border border-emerald-200 bg-emerald-50/60 p-5">
	...
	{#if pe2Error}
		<div class="mb-3 rounded-md border border-danger-border bg-danger-bg p-3 text-sm text-danger">
			{pe2Error}
		</div>
	{/if}
			...
			{#each filteredPe2Sections as section (section.code + '-' + section.status)}
				...
				>
					<div class="min-w-0 flex-1">
						<p
							class="truncate font-mono text-sm font-semibold {section.status === 'gone'
