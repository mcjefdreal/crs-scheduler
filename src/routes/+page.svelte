<script lang="ts">
	...
	import { parseCRSHtml, generateSchedules, db, sectionScore, type Course, type Schedule, type ScheduleResult, type Section, type ScheduleOptions } from '$lib';
	...
	let diffData = $state<{
		...
		changed: Array<{ crn: number; code: string; courseName: string; slotsLeft: number; oldSlotsLeft: number; capacity: number; oldCapacity: number }>;
	...
	function loadPrefs() {
		...
		if (off) { try { daysOff = JSON.parse(off); } catch { daysOff = []; } }
		const excl = localStorage.getItem('excludedInstructors');
		if (excl) { try { excludedInstructors = JSON.parse(excl); } catch { excludedInstructors = []; } }
		const locked = localStorage.getItem('lockedCrns');
		if (locked) { try { lockedCrns = JSON.parse(locked); } catch { lockedCrns = []; } }
	}
	...
	function toggleLock(crn: number) {
		lockedCrns = lockedCrns.includes(crn) ? lockedCrns.filter(c => c !== crn) : [...lockedCrns, crn];
		saveLockedCrns();
	}
	...
	async function refreshAllCourses() {
		...
		const oldSections = new Map<number, { code: string; courseName: string; slotsLeft: number; capacity: number }>();
		for (const course of courses) {
			for (const section of course.sections) {
				oldSections.set(section.crn, { code: section.code, courseName: course.name, slotsLeft: section.slotsLeft, capacity: section.capacity });
			}
		}
		...
		const newSections = new Map<number, { code: string; courseName: string; slotsLeft: number; capacity: number }>();
		for (const course of courses) {
			for (const section of course.sections) {
				newSections.set(section.crn, { code: section.code, courseName: course.name, slotsLeft: section.slotsLeft, capacity: section.capacity });
			}
		}

		const added: Array<{ crn: number; code: string; courseName: string }> = [];
		const removed: Array<{ crn: number; code: string; courseName: string }> = [];
		const changed: Array<{ crn: number; code: string; courseName: string; slotsLeft: number; oldSlotsLeft: number; capacity: number; oldCapacity: number }> = [];
		...
		for (const [crn, info] of newSections) {
			if (!oldSections.has(crn)) {
				...
				if (info.slotsLeft !== old.slotsLeft || info.capacity !== old.capacity) {
					changed.push({ crn, code: info.code, courseName: info.courseName, slotsLeft: info.slotsLeft, oldSlotsLeft: old.slotsLeft, capacity: info.capacity, oldCapacity: old.capacity });
				}
			}
		}
...
<button
      						...
      						{#if showExcluded}
     							<ul class="max-h-64 space-y-1 overflow-y-auto">
        								{#each excludedSections as section}
       									<li class="flex items-center justify-between rounded border border-amber-200 bg-white px-2 py-1.5 text-xs {lockedCrns.includes(section.crn) ? 'border-l-2 border-blue-300 bg-blue-50' : ''}">
			...
			<aside class="space-y-6">
				...
				<section class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
					...
					<div>
						<label class="mb-1 block text-sm font-medium text-slate-700">Minimum gap between classes</label>
						<input
							type="number"
							min="0"
							max="120"
						...
						<div class="flex gap-1">
							{#each ['M', 'T', 'W', 'TH', 'F', 'S'] as day, i}
								<button
									...
									class="rounded-lg px-3 py-1.5 text-xs font-medium transition {daysOff.includes(i) ? 'bg-blue-600 text-white' : 'border border-slate-300 bg-white text-slate-600 hover:bg-slate-100'}"
								>
									{day}
								</button>
							{/each}
						</div>
						<p class="mt-1 text-xs text-slate-500">Schedules with classes on selected days get a score penalty</p>
					</div>
					...
					<div>
						...
						{/if}
						<p class="mt-1 text-xs text-slate-500">Sections taught by excluded instructors will be skipped</p>
					</div>
				</section>


				{#if lockedCrns.length > 0}
				<div class="flex items-center justify-between rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm">
					<span class="font-medium text-blue-700">{lockedCrns.length} section{lockedCrns.length === 1 ? '' : 's'} locked</span>
					<button
						onclick={clearLocks}
			...
			<section class="space-y-6">
				...
				{#if lockedConflict && schedules.length === 0}
					<div class="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">
						...
						<p class="mt-1">Two or more locked sections have overlapping schedules or violate the minimum gap. Unlock or remove conflicting sections to generate schedules.</p>
					</div>
				{/if}
				...
				{#if schedules.length > 0}
					<div class="mb-2 flex items-center justify-between">
						...
						<div class="flex items-center gap-3">
							<span class="text-sm text-slate-500">
								{scheduleFilter.trim() ? `${filteredSchedules.length}/${schedules.length}` : schedules.length} result{schedules.length === 1 ? '' : 's'}
							</span>
					...
					<div class="relative mb-3">
						...
						<input
							...
							class="w-full rounded-lg border border-slate-300 py-2 pl-10 pr-3 text-sm outline-none ring-blue-500/20 transition focus:border-blue-500 focus:ring-4"
						/>
					</div>
					...
					{#if filteredSchedules.length === 0}
						<div class="flex h-32 flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 bg-slate-50 text-center">
								...
								<div class="border-t border-slate-200 px-5 py-4">
									...
									<div class="grid gap-3 sm:grid-cols-2">
										{#each schedule.sections as section}
											<div class="rounded-lg border border-slate-200 p-3 {lockedCrns.includes(section.crn) ? 'border-l-2 border-blue-300 bg-blue-50' : ''}">
												<div class="flex items-start justify-between">
													...
													<div class="flex items-center gap-2">
														<button
															onclick={() => toggleLock(section.crn)}
															class="rounded p-1 text-slate-400 hover:bg-slate-200 hover:text-slate-600"
															title={lockedCrns.includes(section.crn) ? 'Unlock section' : 'Lock section'}
														>
															{#if lockedCrns.includes(section.crn)}
	...
	{#if showDiff}
		<RefreshDiff added={diffData.added} removed={diffData.removed} changed={diffData.changed} onclose={() => showDiff = false} />
	{/if}
		...
		>
			...
			>
				...
				<ol class="list-decimal space-y-2 pl-5 text-sm text-slate-600">
					<li><strong>Add courses:</strong> Enter a course name, paste the full HTML of the CRS section table (right-click → Inspect → copy <code>&lt;table&gt;</code>), and click Add Course. Or use <strong>Fetch</strong> with the course name to auto import. Separate multiple courses with commas for batch fetch (e.g. <em>Eng 13, Math 21</em>).</li>
